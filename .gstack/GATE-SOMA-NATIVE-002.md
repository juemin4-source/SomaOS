...
GATE-SOMA-NATIVE-002 已证明：Capability Policy 与 Claim Adjudication 已接入真实模型和真实工具执行链，并能在模型缺少规定验证证据时，阻止任务进入"已证明完成"状态。

实验同时暴露了治理层的实际成本：过严 Policy（如 R2 中绝对路径误拒）会阻断本可成功的任务。跨 Organ 绕过、Evidence Freshness 和无关修改裁决仍需根据真实执行轨迹确认。

**当前结果证明治理机制真实生效，但尚未证明它在整体成功率和使用成本上全面优于 Baseline。**
