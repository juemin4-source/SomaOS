#!/usr/bin/env node
/**
 * github-eye — handler.js
 *
 * GitHub API 连接器。通过 api/connector.js 处理认证和请求。
 *
 * 输入: { token?, action, repo?, owner?, ... }
 * 动作: repos | issues | pulls | commits | search | rate-limit
 *
...
function main() {
  ...
  if (a && a !== '--') { try { i = JSON.parse(require('fs').readFileSync(path.resolve(a), 'utf-8')); } catch (e) { return out('ERROR', 'Read: ' + e.message); } }
...
async function h(input) {
  ...
  let api;
  try { api = require('./api/connector'); }
  catch { return out('ERROR', 'API connector not found'); }
  ...
  try {
    switch (action) {
      case 'rate-limit': {
        const data = await api.get(`${BASE}/rate_limit`, headers);
      ...
      case 'repo': {
        ...
        const data = await api.get(`${BASE}/repos/${full}`, headers);
      ...
      case 'issues': {
        ...
        const data = await api.get(`${BASE}/repos/${owner}/${repo}/issues?state=${state}&per_page=${input.limit || 10}`, headers);
      ...
      case 'prs': {
        if (!(owner && repo)) return out('ERROR', 'owner and repo required');
        const state = input.state || 'open';
        const data = await api.get(`${BASE}/repos/${owner}/${repo}/pulls?state=${state}&per_page=${input.limit || 10}`, headers);
      ...
      case 'commits': {
        if (!(owner && repo)) return out('ERROR', 'owner and repo required');
        const data = await api.get(`${BASE}/repos/${owner}/${repo}/commits?per_page=${input.limit || 10}`, headers);
      ...
      case 'search': {
        ...
        const data = await api.get(`${BASE}/search/code?q=${encodeURIComponent(input.query)}&per_page=${input.limit || 5}`, headers);
...
function out(r, s, d) { console.log(JSON.stringify({ softill: 'github-eye', result: r, summary: s, data: d || {}, evidence: [] }, null, 2)); process.exit(r === 'PASS' ? 0 : 1); }
