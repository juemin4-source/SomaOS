...
function run(input) {
  ...
  console.log(JSON.stringify({ softill: 'load-prompt', result: 'PASS', summary: 'ok', data: result, evidence: ev, meta: { name: 'load-prompt', level: 'L0_read_probe', v: '0.2.0' } }, null, 2));
  process.exit(0);
}

function fail(msg) {
  console.log(JSON.stringify({ softill: 'load-prompt', result: 'ERROR', summary: msg, data: {}, evidence: [] }));
  process.exit(1);
}
